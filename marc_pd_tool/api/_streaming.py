# marc_pd_tool/api/_streaming.py

"""Streaming mixin for processing very large MARC datasets"""
...
class StreamingAnalyzerProtocol(Protocol):
    """Protocol defining required attributes for StreamingMixin"""
...
class StreamingMixin:
    """Mixin for streaming processing of very large datasets"""
    ...
    def _process_streaming_parallel(
        ...
        try:
            ...
            with Pool(
                ...
                for result in pool.imap_unordered(process_batch, batch_infos):
                    ...
                    completed_batches += 1
                    total_reg_matches += int(batch_stats.get("registration_matches_found", 0))
                    total_ren_matches += int(batch_stats.get("renewal_matches_found", 0))
                    ...
                    batch_duration_str = format_time_duration(batch_stats.get("processing_time", 0))
        ...
        total_skipped_no_year = sum(
            int(stats.get("skipped_no_year", 0)) for stats in all_stats if isinstance(stats, dict)
        )
        self.results.statistics["skipped_no_year"] = int(total_skipped_no_year)

        # Log final performance stats
        total_time = time() - start_time
        total_records = sum(
            stats.get("marc_count", 0) for stats in all_stats if isinstance(stats, dict)
        )
