# marc_pd_tool/application/processing/text_processing.py
...
from marc_pd_tool.shared.utils.text_utils import normalize_text_comprehensive
...
# Lazy loading of abbreviations
_PUBLISHING_ABBREVIATIONS: dict[str, str] | None = None


def _get_abbreviations() -> dict[str, str]:
    """Get abbreviations from config (lazy loading)"""
    global _PUBLISHING_ABBREVIATIONS
    if _PUBLISHING_ABBREVIATIONS is None:
        _config = get_config()
        _PUBLISHING_ABBREVIATIONS = _config.abbreviations
        if not _PUBLISHING_ABBREVIATIONS:
            _PUBLISHING_ABBREVIATIONS = {}  # Default to empty dict instead of raising
    return _PUBLISHING_ABBREVIATIONS
    ...
    for word in words:
        ...
        if clean_word in abbreviations:
            ...
            if word.endswith(".") or len(clean_word) < 5:
                expanded = abbreviations[clean_word]
