...
2. **Author Match**: Fuzzy string matching with name normalization
3. **Year Match**: Exact or ±1 year tolerance
4. **Combined Score**: Weighted average must exceed threshold
...
2. **`application/processing/matching_engine.py`**: Core matching logic
3. **`infrastructure/persistence/_marc_loader.py`**: MARC streaming/batching
4. **`application/processing/parallel_indexer.py`**: Index building
5. **`adapters/api/_analyzer.py`**: Main API entry point
6. **`application/models/analysis_results.py`**: Results storage/export
...
2. **Batch Processing**: Work in chunks of ~1000 records
3. **Parallel When Possible**: Use all CPU cores for computation
4. **Cache Aggressively**: Indexes are expensive to build
5. **Fail Gracefully**: Always cleanup temp files on error
6. **Platform Aware**: Handle Linux/macOS differences explicitly

## ⚡ Quick Debugging Guide

### Application Hangs
1. Check multiprocessing mode (fork vs spawn)
2. Verify indexes are being shared, not reloaded
3. Look for signal handler issues

### Empty Output
1. Check result file registration
2. Verify batch processing completed
3. Check for exceptions in worker processes
...
2. Use JSONDict/JSONList for JSON data
...
2. **Read this `ARCHITECTURE.md`** for technical details
3. **Check `docs/PIPELINE.md`** for processing logic
4. **Always run tests** before considering work complete
5. **Never break the streaming pattern** - memory efficiency is critical
6. **Respect platform differences** - especially macOS spawn vs fork
7. **Register all result files** - or exports will be empty
