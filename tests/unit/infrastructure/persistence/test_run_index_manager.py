# tests/unit/infrastructure/persistence/test_run_index_manager.py
...
class TestRunIndexManager:
    ...
    def test_get_next_run_index_empty(self):
        """Test getting next run index with no existing index file"""
        with TemporaryDirectory() as temp_dir:
            manager = RunIndexManager(temp_dir)
            # Don't initialize index - simulate no existing file
            next_index = manager.get_next_run_index()
            assert next_index == 1

    def test_get_next_run_index_with_runs(self):
        """Test getting next run index with existing runs"""
        with TemporaryDirectory() as temp_dir:
            manager = RunIndexManager(temp_dir)
            manager._initialize_index()

            # Add runs with run index in filename
            run1 = {
                "log_file": "marc_pd_20250101_120000_run001.log",
                "marc_count": 100,
                "status": "completed",
            }
            manager.add_run(run1)

            run2 = {
                "log_file": "marc_pd_20250101_130000_run005.log",
                "marc_count": 200,
                "status": "completed",
            }
            manager.add_run(run2)

            # Should return max + 1
            next_index = manager.get_next_run_index()
            assert next_index == 6

    def test_get_next_run_index_mixed_formats(self):
        """Test getting next run index with mixed filename formats"""
        with TemporaryDirectory() as temp_dir:
            manager = RunIndexManager(temp_dir)
            manager._initialize_index()

            # Add runs with different formats
            runs = [
                {"log_file": "marc_pd_20250101_120000_run003.log", "marc_count": 100},
                {"log_file": "marc_pd_20250101_130000.log", "marc_count": 200},  # No run index
                {"log_file": "custom_run010.log", "marc_count": 300},
                {"log_file": "test_run002.log", "marc_count": 400},
            ]

            for run in runs:
                manager.add_run(run)

            # Should find the max run index (10) and return 11
            next_index = manager.get_next_run_index()
            assert next_index == 11

    def test_get_next_run_index_invalid_numbers(self):
        """Test getting next run index with invalid run numbers"""
        with TemporaryDirectory() as temp_dir:
            manager = RunIndexManager(temp_dir)
            manager._initialize_index()

            # Add runs with invalid run numbers
            runs = [
                {"log_file": "marc_pd_20250101_120000_run001.log", "marc_count": 100},
                {"log_file": "marc_pd_20250101_130000_runABC.log", "marc_count": 200},  # Invalid
                {"log_file": "test_run.log", "marc_count": 300},  # No number
            ]

            for run in runs:
                manager.add_run(run)

            # Should only consider valid run number (1) and return 2
            next_index = manager.get_next_run_index()
            assert next_index == 2
