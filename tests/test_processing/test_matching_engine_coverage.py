...
class TestDataMatcherIgnoreThresholds:
    ...
    def test_find_best_match_ignore_thresholds_lccn_match(self):
        ...
        assert result['similarity_scores']['combined'] == 100
...
class TestDataMatcherRegularMatching:
    ...
    def test_find_best_match_early_exit(self):
        ...
        with patch.object(matcher.similarity_calculator, 'calculate_title_similarity', return_value=100.0):
            with patch.object(matcher.similarity_calculator, 'calculate_author_similarity', return_value=100.0):
                result = matcher.find_best_match(
                    marc_pub,
                    copyright_pubs,
                    year_tolerance=2,
        ...
        assert result is not None
        assert result['copyright_record']['source_id'] == "c001"  # Should be first match due to early exit
...
class TestDataMatcherHelperMethods:
    ...
    def test_create_match_result_lccn(self):
        ...
        assert result['is_lccn_match'] == True
        assert all(result['similarity_scores'][field] == 100.0 for field in ['title', 'author', 'publisher', 'combined'])
...
class TestWorkerFunctions:
    ...
    def test_init_worker_with_cached_data(self, tmp_path):
        ...
        with patch('marc_pd_tool.processing.matching_engine.CacheManager') as mock_cache_class:
            with patch('marc_pd_tool.processing.matching_engine._worker_data', {}) as mock_worker_data:
                mock_cache = Mock()
