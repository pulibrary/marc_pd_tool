...
class TestLargeFileHandling:
    ...
    def large_publication_set(self) -> list[Publication]:
        ...
        for i in range(1000):
            ...
            pub = Publication(
                ...
                country_classification=CountryClassification.US if i % 2 == 0 else CountryClassification.NON_US
            )
            pubs.append(pub)
        return pubs
    ...
    def test_memory_usage_during_processing(
        self,
        large_publication_set: list[Publication],
        ...
        analyzer = MarcCopyrightAnalyzer(
            cache_dir=str(temp_output_dir / "memory_test")
        )
    ...
    def test_batch_size_performance(
        ...
        for config in batch_configs:
            analyzer = MarcCopyrightAnalyzer(
                cache_dir=str(temp_output_dir / config["name"])
            )
    ...
    def test_export_performance_large_dataset(
        ...
        analyzer = MarcCopyrightAnalyzer(
            cache_dir=str(temp_output_dir / "export_test")
        )
    ...
    def test_incremental_processing(
        ...
        analyzer = MarcCopyrightAnalyzer(
            cache_dir=str(temp_output_dir / "incremental_cache")
        )
